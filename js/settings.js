$('#inputPasswordConfirm').on('keyup', function () {
    ...
    if (!(firstField.val().isEmptyString())) {
        ...
        $(this).parent().parent().toggleClass('has-success', inspect).toggleClass('has-error', !inspect);
        firstField.parent().parent().toggleClass('has-success', inspect);
    } else {
        $(this).parent().parent().removeClass('has-success has-error');
        firstField.parent().parent().removeClass('has-success');
    }
});
